...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const jp = await db.jenisPelanggaran.update({
      where: { id },
      data: { nama: data.nama, poin, kategori },
    })
    return NextResponse.json(jp)
  } catch (e) {
    return prismaErrorResponse(e, {
      notFound: 'Jenis pelanggaran tidak ditemukan',
      fallback: 'Gagal memperbarui jenis pelanggaran',
    })
  }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  try {
    await db.jenisPelanggaran.delete({ where: { id } })
